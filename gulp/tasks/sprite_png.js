...
function deleteSpritePNG() {
  // delete old sprite
  del.sync([
    `${paths.source.images}/sprites/sprite_png*.*`,
    `${paths.destination.images}/sprites/sprite_png*.*`,
  ]);
}

function createSpritePNG() {
  const fileName =
    'sprite_png_' + Math.random().toString(16).substring(2, 10) + '.png';

  const spriteData = gulp.src(`${paths.source.iconsPNG}/*.png`).pipe(
    gulpSpritesmith({
      imgName: fileName,
      ...
      cssVarMap: function (sprite) {
        sprite.name = 'icon-' + sprite.name.replace(/ /g, '_____');
      },
    }),
  );
  spriteData.css.pipe(gulp.dest(`${paths.source.styles}/sprites`));
  spriteData.img.pipe(gulp.dest(`${paths.source.images}/sprites`));
...
export { task_sprite_png, watch_sprite_png };
